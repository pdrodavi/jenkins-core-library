...
class StringItem implements Item, Serializable {
   ...
   */
  private static final String RELEASE_VERSION_INDEX = "${_QUALIFIERS.indexOf('')}"
  ...
  int compareTo(Item item) {
    if (item == null) {
      // 1-rc < 1, 1-ga > 1
      return comparableQualifier(value) <=> RELEASE_VERSION_INDEX
    }
    switch (item.getType()) {
      case INTEGER_ITEM:
        return -1; // 1.any < 1.1 ?
      ...
      case STRING_ITEM:
        return comparableQualifier(value) <=> comparableQualifier(((StringItem) item).value)
